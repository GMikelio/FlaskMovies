...
def account():
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        if form.profile_image.data:
            ...
            ## Διαγραφή του προηγούμενου profile_image, από το δίσκο image_prv_
            profile_img_reject = os.path.join(app.root_path, 'static/images/profiles_images', current_user.profile_image)
            if os.path.isfile(profile_img_reject): 
                os.remove(profile_img_reject) 

            ## Αποθήκευση του νέου profile_image
...
def new_movie():
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        plot = form.plot.data
        rls_year = form.release_year.data
        rating = form.rating.data

        if form.image.data:
            try:
                img_file = image_save(form.image.data, 'movies_images', (640, 640))
            except:
                abort(415)

            movie = Movie(title=title, plot=plot, release_year=rls_year, rating=rating, author=current_user, image=img_file)
        else:
            movie = Movie(title=title, plot=plot, release_year=rls_year, rating=rating, author=current_user)
...
@app.route("/movies_by_author/<ordering_by>")
@app.route("/movies_by_author/<int:author_id>")
def movies_by_author(author_id=0, ordering_by=None):
    ...
    if movie:
        ...
        if request.method == 'POST' and form.validate_on_submit():
            ...
            if form.image.data:
                ...
                ## Διαγραφή της προηγούμενης image, από το δίσκο
                image_prv_reject = os.path.join(app.root_path, 'static/images/movies_images', movie.image)
                if os.path.isfile(image_prv_reject): 
                    os.remove(image_prv_reject) 
...
def delete_movie(movie_id):
    ...
    if movie:
        ...
        ## Διαγραφή της image, από το δίσκο
        image_prv_reject = os.path.join(app.root_path, 'static/images/movies_images', movie.image)
        if os.path.isfile(image_prv_reject): 
            os.remove(image_prv_reject) 
