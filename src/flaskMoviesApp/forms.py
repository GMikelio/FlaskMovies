...
class NewMovieForm(FlaskForm):
    ...
    plot = TextAreaField(label="Υπόθεση",
                        validators=[DataRequired(message="Αυτό το πεδίο δε μπορεί να είναι κενό."), 
                                    Length(min=5, message="Η υπόθεση της ταινίας πρέπει να έχει τουλάχιστον 5 χαρακτήρες")])
    
    ## Αρχείο Εικόνας, με επιτρεπόμενους τύπους εικόνων τα 'jpg', 'jpeg', 'png', και μέγιστο μέγεθος αρχείου εικόνας τα 2 MBytes, ΜΗ υποχρεωτικό πεδίο
    image = FileField('Εικόνα', validators=[Optional(strip_whitespace=True),
                                            FileAllowed([ 'jpg', 'jpeg', 'png' ],
                                            'Επιτρέπονται μόνο αρχεία εικόνων τύπου jpg, jpeg και png!'),
                                            maxImageSize(max_size=2)])
    ...
    release_year = IntegerField(label='Έτος πρώτης προβολής', validators=[NumberRange(min=1888, max=current_year)])
    ...
    rating = IntegerField(label='Βαθμολογία', validators=[NumberRange(min=1, max=100)])
